"""
Standalone batched multi-hit job for mf_ls (light-w, strange-v, Gamma5,
...
def build_job(hits=None):
    ...
    run_id = "mf-ls-multihit-" + "".join(f"h{h}" for h in hits)
    job = Job(run_id)
    pool = VectorPool(job)
    ...
    job.add(M.a2a_meson_field(
        ...
        lw, sv, f"mf/{name}", config.GAMMA5, config.KAON_MOM))
    ...
    for width_tag, alpha, N in config.SMEAR_WIDTHS:
        ...
        ssv_name = f"a2a_s_v_multihit_{width_tag}"
        job.add(M.a2a_covariant_smear(
            ssv_name, a2a_vectors=sv, gauge="gauge_APE", alpha=alpha, N=N,
            orthog_axis=config.ORTHOG_AXIS, output="", multi_file=False))

        name = f"mf_ls_multihit_{width_tag}"
        job.add(M.a2a_meson_field(
            name, config.BLOCK_STRANGE_LEG, config.CACHE_BLOCK_MF,
            slw_name, ssv_name, f"mf/{name}", config.GAMMA5, config.KAON_MOM))
...
def main():
    ...
    print(f"wrote 1 mf_ls multihit job ({len(DEFAULT_HITS)} hits) to {out_dir}")
